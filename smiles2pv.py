...
from bk_dataset import SMILESDataset_pretrain, my_collate_fn
from dataset import MultiLMDBDataset, tri_collate_fn, ValLMDBDataset
#from dataset import MultiLMDBDataset, tri_collate_fn 
...
def pv_generate(model, data_loader):
    ...
    if isinstance(data_loader, list):
        ...
        prop_input = model.prop_cls.expand(len(data_loader), -1, -1)
        prediction = []
        for _ in range(53):
            output = generate(model, prop_input, text_embeds, text_input.attention_mask[:, 1:])
            prediction.append(output)
            output = model.prop_embed(output.unsqueeze(2))
    ...
    #for (prop, text) in tqdm(data_loader, total=len(data_loader)):
...
#        dist_atts = torch.ones(dist_embeds.size()[:-1], dtype=torch.long, device=device)
        ...
        # Sub-option 1: K/V concat — text는 raw 유지, dist 토큰만 append
...
#    dataset_test = MultiLMDBDataset(['test.lmdb'])
